...
import medicines from '../../../../Backend/api/medicines.api';
...
function MedicineForm() {
    ...
    const onSubmit = async (data) => {
        console.log(data)
        try {
            const res = await axios.post("http://localhost:3000/api/orders/medicine/insert", data)
    ...
    return (
        ...
        >
            ...
            <div className='rounded-2xl shadow-xl flex justify-center flex-col w-1/2 mx-auto py-10 bg-white'>
                ...
                <h1 className='text-center font-bold text-2xl text-rose-500 mb-9'>Medicine Request Form</h1>
                ...
                <form action="" onSubmit={handleSubmit(onSubmit)} >
                    <div className='grid gap-3 mx-16'>
                        ...
                        <div className="">
                            <label className='bloodGroup font-semibold pr-20'>Medicine</label>
                            <Controller
                                name="generic_name"
                                control={control}
                                defaultValue=""
                                render={({ field }) => (
                                    <select {...field} id="medicine"
                                        {...register("generic_name", { required: true })}
                                        className='outline-none py-1 px-2 w-full rounded border focus:border-2 focus:border-blue-600 my-2'
                                    >
                                        <option value="">Select a medicine</option>
                                        {medicines.map((medicine, index) => (
                                            <option key={index} value={medicine.generic_name}>
